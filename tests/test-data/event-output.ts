...
export const eventDescriptions: { [signatureHash: string]: EventDescription } = {
	'5c4cf109e00bf95f1fe07fc3173b24b6e8f94894407c6ec23c3e5fb82419a6ce': {"name":"Durian","signature":"Durian(uint256,string,bytes,address)","parameters":[{"indexed":false,"name":"a","type":"uint256"},{"indexed":true,"name":"b","type":"string"},{"indexed":false,"name":"c","type":"bytes"},{"indexed":true,"name":"d","type":"address"}]}
}
...
export type Event<TLargeInteger> = DecodedEvent<TLargeInteger> | Banana.Durian<TLargeInteger>
...
export class Banana<TLargeInteger> extends Contract<TLargeInteger> {
	...
	public cherry = async(options?: { attachedEth?: UInt256<TLargeInteger> }): Promise<Array<Event<TLargeInteger>>> => {
		options = options || {}
		...
		return await this.remoteCall(signatureHash, inputParameterDescriptions, [], { transactionName: 'cherry' }, options.attachedEth)
	}

	public cherry_ = async(options?: { attachedEth?: UInt256<TLargeInteger> }): Promise<void> => {
		options = options || {}
		const signatureHash = Uint8Array.from([ 0xa2, 0x74, 0xc8, 0x6b ]) as SignatureHash
		const inputParameterDescriptions: Array<ParameterDescription> = []
		const outputParameterDescriptions: Array<ParameterDescription> = []
		await this.localCall(signatureHash, inputParameterDescriptions, outputParameterDescriptions, [], options.attachedEth)
	}
}
