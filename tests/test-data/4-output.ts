...
export class banana<TLargeInteger> extends Contract<TLargeInteger> {
	...
	public cherry = async (durian: UInt256<TLargeInteger>, attachedEth?: UInt256<TLargeInteger>): Promise<Array<Event<TLargeInteger>>> => {
		const signatureHash = SignatureHash.fromByteArray([ 0x5a, 0x6f, 0x90, 0x20 ])
	...
	public cherry_ = async (durian: UInt256<TLargeInteger>, attachedEth?: UInt256<TLargeInteger>): Promise<boolean> => {
		const signatureHash = SignatureHash.fromByteArray([ 0x5a, 0x6f, 0x90, 0x20 ])
		const inputParameterDescriptions: Array<ParameterDescription> = [{"name":"durian","type":"uint256"}]
		...
		return <boolean>result.eggplant
	}
}
