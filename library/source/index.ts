...
${
	abiEvent.inputs
		.map(parameter => (parameter.indexed && parameter.type === 'string') ? Object.assign({}, parameter, {type: 'bytes32'}) : parameter)
		.map(input => `\t\t\t${input.name}: ${toTsTypeString(input, { contractName: contractName, functionName: abiEvent.name })}`).join('\n')
}
		}
	}
}`
...
function remoteMethodTemplate(abiFunction: AbiFunction, errorContext: { contractName: string }) {
	...
	const separator = (abiFunction.inputs.length !== 0 && abiFunction.payable) ? ', ' : ''
	const attachedEthInputParameter = abiFunction.payable ? 'attachedEth?: UInt256<TLargeInteger>' : ''
	const attachedEthCallParameter = abiFunction.payable ? ', attachedEth' : ''
	return `	public ${abiFunction.name} = async (${params}${separator}${attachedEthInputParameter}): Promise<Array<Event<TLargeInteger>>> => {
		const signatureHash = SignatureHash.fromByteArray(${signatureArrayString})
		const inputParameterDescriptions: Array<ParameterDescription> = ${JSON.stringify(abiFunction.inputs)}
		return await this.remoteCall(signatureHash, inputParameterDescriptions, [${argNames}], { transactionName: '${abiFunction.name}' }${attachedEthCallParameter})
	}`
}
...
function localMethodTemplate(abiFunction: AbiFunction, errorContext: { contractName: string }) {
	...
	const returnValue: string = (abiFunction.outputs.length === 1)
		? `<${returnType}>result.${abiFunction.outputs[0].name || 'result'}`
		: `<${returnType}>result`
	return `	public ${abiFunction.name}_ = async (${params}${separator}${attachedEthInputParameter}): Promise<${returnType}> => {
		...
		${abiFunction.outputs.length !== 0 ? 'const result = ' : ''}await this.localCall(signatureHash, inputParameterDescriptions, outputParameterDescriptions, [${argNames}]${attachedEthCallParameter})${abiFunction.outputs.length !== 0 ? `\n\t\treturn ${returnValue}` : ''}
	}`
}
...
function toTsTypeString(abiParameter: AbiParameter, errorContext: { contractName: string, functionName: string }): string {
	if (/^u?int(\d*)$/.test(abiParameter.type)) {
		...
		const size = Number.parseInt(match[2])
		const suffix = (size > 32) ? '<TLargeInteger>' : ''
		return `${prefix}Int${size}${suffix}`
